...
trait AIPlayer:
   ...
   *
   * @return
   * a tuple where the first element is the [[AIPlayerAction]] taken and the second is optionally
   * a pair of [[City]] representing the route to claim when the decision is [[AIPlayerAction.CLAIM_ROUTE]].
   */
...
object AIPlayer:
  ...
  private case class AIPlayerImpl(chatModel: ChatModel, player: Player, gameMap: GameMap) extends AIPlayer:
    ...
    private def getUnclaimedRoutes: Set[(City, City)] = gameMap.routes
        .filter(route => gameMap.getPlayerClaimingRoute((route.connectedCities._1.name, route.connectedCities._2.name))
          .exists(_.isEmpty))
